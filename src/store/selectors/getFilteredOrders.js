...
export const filteredOrders = (state) => {
  const data = orders(state);
  const search = searchValue(state);
  const dateFrom = dateFromValue(state);
  const dateTo = dateToValue(state);
  const status = statusValue(state);
  const priceFrom = priceFromValue(state);
  const priceTo = priceToValue(state);
